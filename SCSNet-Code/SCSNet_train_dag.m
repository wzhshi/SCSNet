function [net,stats] = SCSNet_train_dag(net, imdb, getBatch, varargin)
...
opts.expDir = 'results';%fullfile('data','exp') ;
opts.continue = true ;
opts.batchSize = 256 ;
...
%     state.params(p).wzh_mean = zeros(size(net.params(p).value));
%     state.params(p).wzh_variance = zeros(size(net.params(p).value));
% end
opts.errorFunction = 'pdist' ;
opts.errorFunction = @error_pdist ;
opts.errorLabels = {'pdisterr'};
...
function err = error_pdist(res)
% predictions = gather(res(end-1).x) ;
% err = mean( mean( mean(mean(sqrt((predictions - labels).^2), 4), 3), 2) );
err = mean(reshape(gather(res.vars(end).value),[],1));
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    %---------wzhshi
    error = sum([error, [...
        reshape(opts.errorFunction(net),[],1)
        reshape(opts.errorFunction(net),[],1) ; ]],2) ;
...
%   stats = opts.extractStatsFn(net) ;
    %---------wzhshi
  stats.objective = error(1)/num ;
  for i = 1:numel(opts.errorLabels)
    stats.(opts.errorLabels{i}) = error(i+1)/num ;
  end
...
lossAvg = gather(net.layers(end).block.average);
% losszuo = gather(net.layers(end).block.loss_zuo);
% lossLas = gather(net.layers(end).lastPred);
...
%   fprintf(' lossAvg: %.6f losszuo: %.6f', lossAvg,losszuo);
  fprintf(' lossAvg: %.6f', lossAvg);
  fprintf('\n') ;
end
...
for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
    ...
    case 'gradient'
      ...
      if net.meta.adjGradClipping
          threshold = net.meta.gradthresh /thisLR;
      state.momentum{p} = opts.momentum * state.momentum{p} ...
        - thisDecay * net.params(p).value ...
        - (1 / batchSize) * gradientClipping(net.params(p).der,threshold);
      else
           state.momentum{p} = opts.momentum * state.momentum{p} ...
        - thisDecay * net.params(p).value ...
 ...
 for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
    ...
    case 'gradient'
      ...
      state.params(p).t = state.params(p).t + 1;
      t = state.params(p).t;
      alpha = thisLR;
      lr = alpha * sqrt(1 - opts.beta2^t) / (1 - opts.beta1^t);
      state.params(p).m = state.params(p).m + (1 - opts.beta1) .* (net.params(p).der - state.params(p).m);
      state.params(p).v = state.params(p).v + (1 - opts.beta2) .* (net.params(p).der .* net.params(p).der - state.params(p).v);
      
      if net.meta.adjGradClipping
            theta = net.meta.gradthresh /lr;
            net.params(p).value = net.params(p).value - lr * gradientClipping(state.params(p).m ./ (sqrt(state.params(p).v) + opts.epsilon),theta);
       else
            net.params(p).value = net.params(p).value - lr * state.params(p).m ./ (sqrt(state.params(p).v) + opts.epsilon);
      end
